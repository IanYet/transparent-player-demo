...
function processPerspective2(srcImageData: ImageData) {
	...
	const tarImageData = offCtx.createImageData(srcImageData.width, srcImageData.height)
...
function processClip(imageData: ImageData, clipImageData: Uint8Array) {
	const { offCtx } = context
	const tarImageData = offCtx.createImageData(imageData.width, imageData.height)
